...
type (
	IDevCategory interface {
		...
		// ListForPage 产品分类列表
		ListForPage(ctx context.Context, page int, limit int, name string) (out []*model.ProductCategoryTreeOutput, total int, err error)
	...
	IDevDeviceTree interface {
		...
		// Change 更换上下级
		Change(ctx context.Context, infoId int, parentInfoId int) error
		// Detail 信息详情
